...
const TeacherDashboard = () => {
  ...
  const transparent = <div className=' w-full h-screen bg-gray-200 dark:bg-darkTransparent z-5 absolute' onClick={() => {
    setSubjectClick(false);
  }}></div>;

  let classBox = classes.map((item) => {
    return (
      <div key={item.class_id} className='cursor-pointer flex flex-col space-y-2 border-1 border-white shadow-discordBlue shadow-lg hover:shadow-md hover:shadow-discordBlue p-10 mb-20' onClick={() => onClassBoxClicked(item.class_id)}>
        <h1 className='text-3xl text-discordBlue'>{item.faculty + " " +  item.semester + " " + item.shift}</h1>
      </div>
    )
  })

  let subjectInfoCard = (subjects.length>=1)?<SubjectInfoCard className='z-10' subjectName={subjects[0].name} date={setDate} present={presentStudents} />:null;
  ...
  if(!isAuthenticatedUser()) return null
    ...
    <div className='hidden md:flex md:h-screen md:w-screen  md:justify-between md:overflow-hidden'>
      ...
      <div className='basis-1/6 dark:bg-darkLeftSidebar'>
        {/* Project Logo */}
        {/* <h1 className='text-2xl text-blue-500 mt-6'>NCIT</h1> */}
        <div className='flex justify-center py-6 px-2 items-center cursor-pointer space-x-2 '>
          <img alt='attendance guru logo' src={`${logo}`} className='w-50 h-50' />
        </div>
        <div>
      ...
      {/* middle section */}
      <div className='basis-5/6 bg-gray-100 dark:bg-darkMiddlebar'>
        {/* Searchbar */}
        ...
        {(dashboardClicked)?
        (<div className='h-full overflow-y-scroll mt-6 pb-32'><h1 className='text-2xl text-discordBlue text-center'>Dashboard</h1>

        <div className='my-6'>
          <h3 className='text-lg text-discordBlue text-center mb-1'>Class Attendance Performance</h3>
          <BarChartComp />
        </div>
        ...
        <div>
          <div className='text-center flex justify-center space-x-2'>
            <div className='text-discordBlue'>
              <EmojiEventsIcon />
            </div>
            <h3 className='text-center text-lg text-discordBlue mb-4'>Top Attendees</h3>
          </div>
          {/* list of top attendees */}
          <div className='px-28 text-discordBlue'>
              <TopAttendeesTable />
          </div>
        ...
        {(classesClicked)?(
          ...
          {(isButtonLoading && !classes)?<Bars className='w-5 h-5' stroke='#5765f2' fill='#5765f2' style={{width : '50px', height:'50px'}} />:(subjectClick)?<>{transparent}<div class='flex justify-center bg-black p-10'>{subjectInfoCard}</div></>:null}
          ...
          <div className='flex w-full justify-center space-x-5 flex-wrap relative'>
            ...
            {(isButtonLoading)?<Bars className='w-5 h-5' stroke='#5765f2' fill='#5765f2' style={{width : '50px', height:'50px'}} />:(classes)?classBox:null}
        ...
        {(settingClicked)?(
          <div className='w-full h-full flex justify-center '><Setting notificationClick={onNotificationClick} /></div>
        ):null
        }
    ...
    {/* For smaller Device */}
    <div className='h-screen w-screen no-scrollbar md:hidden dark:bg-darkMiddlebar'>
      ...
      {(showMenu && dashboardClicked)?(
        ...
        <div>
          ...
          {/* list of top attendees */}
          <div className='text-discordBlue overflow-y-scroll'>
              <TopAttendeesTable />
          </div>
      ...
      {(showMenu && classesClicked)?(
        ...
        <h1 className='text-center text-2xl text-discordBlue my-4 mb-8'>ClassNamees</h1>
        <div className='flex justify-center my-4'><Calendar /></div>
        <div className='flex w-full justify-center space-x-5 flex-wrap relative'>
        {/* {(subjectClick)?<>{transparent}<SubjectInfoCard className='z-10' /></>:null} */}
          <div className='flex flex-col space-y-2 border-1 border-white shadow-discordBlue shadow-lg hover:shadow-md hover:shadow-discordBlue p-10 mb-20' onClick={() => setSubjectClick(true)}>
            <h1 className='text-3xl text-discordBlue'>AINN</h1>
          </div>
          <div className='flex flex-col space-y-2 border-1 border-white shadow-discordBlue shadow-lg hover:shadow-md hover:shadow-discordBlue p-10 mb-20'>
            <h1 className='text-3xl text-discordBlue'>WT</h1>
          </div>
          <div className='flex flex-col space-y-2 border-1 border-white shadow-discordBlue shadow-lg hover:shadow-md hover:shadow-discordBlue p-10 mb-20'>
            <h1 className='text-3xl text-discordBlue'>PPL</h1>
          </div>
          <div className='flex flex-col space-y-2 border-1 border-white shadow-discordBlue shadow-lg hover:shadow-md hover:shadow-discordBlue p-10 mb-20'>
            <h1 className='text-3xl text-discordBlue'>OOSD</h1>
          </div>
          <div className='flex flex-col space-y-2 border-1 border-white shadow-discordBlue shadow-lg hover:shadow-md hover:shadow-discordBlue p-10 mb-20'>
            <h1 className='text-3xl text-discordBlue'>EE</h1>
          </div>
        </div>
      </div>
      ):null
      ...
      {(showMenu && settingClicked)?(
        <div className='w-full h-full flex justify-center px-5 '><Setting notificationClick={onNotificationClick} /></div>
      ):null
      }
