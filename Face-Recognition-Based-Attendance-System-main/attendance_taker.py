...
class Face_Recognizer:
    ...
    def process(self, stream):
        ...
        current_time = datetime.datetime.now().strftime('%H:%M:%S')
        attendance_data = {'name': name, 'date': current_date, 'time': current_time}
        with open('present_student.json', 'a') as json_file:
            json.dump(attendance_data, json_file)
            json_file.write('\n')

        print(f"{name} attendance saved to JSON file")
        ...
        if response.status_code == 200:
            print(f"{name} attendance saved to API")
        else:
            print(f"Failed to save {name} attendance to API")
